...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    file_genertor_command = f'ffmpeg -hide_banner -loglevel quiet -progress "{progress}" -i "{video_file}" {watermark[0]}  -c:v {codec[0]}  -map 0 -crf {crf[0]} -c:s copy -pix_fmt yuv420p -s {resolution[0]} -b:v 150k -c:a {audio_codec[0]} -b:a {audio_b[0]} -preset {preset[0]} -metadata title="{metadata_title}" "{out_put_file_name}" -y'
 #Done !!
